...
void ToDoFacade::deleteList()
{
	...
	if (fs::exists("List\\" + (*b)->name + ".txt"))
	{
		fs::remove("List\\" + (*b)->name + ".txt");
	}
...
{
	if (getNoYes("вы уверены что хотите сохранить изменения в файл?"))
	{
...
{
	if (getNoYes("вы уверены что хотите выгрузить из файла?"))
	{
		List.clear();

		if (!fs::exists("List") || fs::is_empty("List\\"))
		{
