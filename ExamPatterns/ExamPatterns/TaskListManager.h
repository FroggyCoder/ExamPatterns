...
void TaskListFacade::addTask()
{
	...
	//getting tag
	{
		...
		if (getNoOrYes("хотите добавить тег?"))
		{
			string Tag;
			cout << "тег: ";
			getline(cin, Tag);
...
{
	if (getNoOrYes("вы уверены что хотите очистить этот список?")) taskList->tasks.clear();
}
